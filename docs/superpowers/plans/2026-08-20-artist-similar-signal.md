...
- Execution worktree: create a SIBLING at `~/kode/sortify-artistsim` (branch `artist-similar`, base master ≥ c96987a) with its OWN venv (`python3 -m venv .venv && .venv/bin/pip install -e '.[dev]'`). Worktrees must be siblings of `~/kode/sortify` or the `sortify/account_ledger.py` relative symlink breaks.
- Do not restart `sortify.service` or touch `~/kode/sortify` during execution.
